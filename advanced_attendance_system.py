        ...
        for (x, y, w, h) in faces:
            ...
            if self.use_facenet:
                ...
                try:
                    ...
                    # Check if similarity is above threshold (use 0.5 for cosine similarity)
                    # Cosine similarity ranges from -1 to 1, where 1 means identical
                    if best_similarity > 0.5:  # Lowered threshold for better matching
                        ...
                        if name not in self.marked_today:
                            ...
                            self.update_info(f"✓ Attendance marked for: {name} (Similarity: {best_similarity:.2f})")
                    else:
                        name = "Unknown"
            ...
            cv2.putText(frame, f"{name} ({confidence_display:.1f})", (x+6, y-6), 
